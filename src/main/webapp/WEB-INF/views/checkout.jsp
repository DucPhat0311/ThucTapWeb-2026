...
<script>
    ...
    async function updateShippingInformation() {
        ...
        if (serviceIdInput) {
            serviceIdInput.value = DEFAULT_SERVICE_ID;
        }


        await Promise.all([
            calculateLeadTime(DEFAULT_SERVICE_ID),
            calculateShippingFee(DEFAULT_SERVICE_TYPE_ID, DEFAULT_SERVICE_ID)
        ]);
    }
    ...
    async function calculateShippingFee(serviceTypeId, serviceId) {
        ...
        const length = parseInt(document.getElementById('total-length').value);
        const width = parseInt(document.getElementById('total-width').value);
        const height = parseInt(document.getElementById('total-height').value);
