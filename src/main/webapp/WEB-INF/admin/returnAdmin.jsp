                    ...
                    <section class="content">
                        ...
                        <main class="page active">
                            <div class="cards return-cards">
                                <a class="card ${empty currentStatus ? 'active' : ''}"
                                    href="${pageContext.request.contextPath}/returnAdmin">
                                    Tổng yêu cầu<span>${total}</span>
                                </a>
                                <a class="card ${currentStatus == 'REQUESTED' ? 'active' : ''}"
                                    href="${pageContext.request.contextPath}/returnAdmin?status=REQUESTED">
                                    Chờ duyệt<span>${countRequested}</span>
                                </a>
                                <a class="card ${currentStatus == 'APPROVED' ? 'active' : ''}"
                                    href="${pageContext.request.contextPath}/returnAdmin?status=APPROVED">
                                    Đã chấp nhận<span>${countApproved}</span>
                                </a>
                                <a class="card ${currentStatus == 'RETURNING' ? 'active' : ''}"
                                    href="${pageContext.request.contextPath}/returnAdmin?status=RETURNING">
                                    Đang hoàn hàng<span>${countReturning}</span>
                                </a>
                                <a class="card ${currentStatus == 'REJECTED' ? 'active' : ''}"
                                    href="${pageContext.request.contextPath}/returnAdmin?status=REJECTED">
                                    Đã từ chối<span>${countRejected}</span>
                                </a>
                                <a class="card ${currentStatus == 'RETURNED' ? 'active' : ''}"
                                    href="${pageContext.request.contextPath}/returnAdmin?status=RETURNED">
                                    ...
                                    <tbody>
                                        ...
                                        <c:forEach items="${returns}" var="returnRequest">
                                            <tr>
                                                ...
                                                <td>
                                                    <c:out value="${returnRequest.customerName}" />
                                                </td>
                                                <td>
                                                    <c:out value="${returnRequest.reasonLabel}" />
                                                </td>
                                                ...
                                                <td>
                                                    <c:if test="${perms['view_detail']}">
                                                        <a href="${pageContext.request.contextPath}/returnAdmin?mode=view&amp;id=${returnRequest.id}"
                                                            class="icon-btn view" title="Xem và xử lý yêu cầu"
                                                            aria-label="Xem và xử lý yêu cầu">
                                                            <i class="fa fa-eye"></i>
                                                        </a>
                                                    </c:if>
                                                </td>
                            ...
                            <c:if test="${totalPages > 1}">
                                ...
                                <div class="pagination">
                                    <div class="pagination-info">
                                        ...
                                        - ${currentPage * pageSize > totalFiltered ? totalFiltered : currentPage *
                                        pageSize}
                                    ...
                                    <div class="pagination-controls">
                                        <c:if test="${currentPage > 1}">
                                            <a class="page-btn"
                                                href="${pageContext.request.contextPath}/returnAdmin?page=1${statusParam}">«
                                                Đầu</a>
                                            <a class="page-btn"
                                                href="${pageContext.request.contextPath}/returnAdmin?page=${currentPage - 1}${statusParam}">‹
                                                Trước</a>
                                        </c:if>
                                        ...
                                        <c:forEach begin="1" end="${totalPages}" var="i">
                                            <c:choose>
                                                ...
                                                <c:when
                                                    ...
                                                    <a class="page-btn"
                                                        href="${pageContext.request.contextPath}/returnAdmin?page=${i}${statusParam}">${i}</a>
                                                </c:when>
                                        ...
                                        <c:if test="${currentPage < totalPages}">
                                            <a class="page-btn"
                                                href="${pageContext.request.contextPath}/returnAdmin?page=${currentPage + 1}${statusParam}">Sau
                                                ›</a>
                                            <a class="page-btn"
                                                href="${pageContext.request.contextPath}/returnAdmin?page=${totalPages}${statusParam}">Cuối
                                                »</a>
                                        </c:if>
                                    </div>
                                </div>
