...
<div class="container">
    ...
    <form action="${pageContext.request.contextPath}/admin/warehouseExportForm" method="post">
        <div class="card">
            <h3>Thông tin phiếu</h3>
            <div class="row">
                <div class="col">
                    <label for="note">Ghi chú</label>
                    <input type="text" id="note" name="note" placeholder="Nhập ghi chú cho phiếu xuất...">
                </div>
            </div>
        </div>
                    ...
                    <div class="col col-product">
                        ...
                        <select name="productVariantId[]" required>
                            ...
                            <c:forEach var="v" items="${variants}">
                                <option value="${v.id}">${v.productName} - Màu: ${v.colorName} - Size: ${v.sizeName} (Tồn: ${v.stock})</option>
                            </c:forEach>
                        </select>
                    </div>
...
<script>
    function addRow() {
        ...
        colPrice.innerHTML = '<label>Đơn giá xuất</label>' +
            '<input type="number" name="price[]" step="0.01" min="0" required placeholder="Đơn giá">';
