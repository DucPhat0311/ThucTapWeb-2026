...
public class InventoryReceiptDaoAdmin extends BaseDao {
    ...
    public InventoryReceiptDaoAdmin() {
        ...
        try {
            getJdbi().useHandle(handle -> {
                ...
                try {
                    handle.execute(
                            "ALTER TABLE inventory_receipt_details ADD COLUMN cost_price DECIMAL(15,2) NULL DEFAULT NULL");
                } catch (Exception e) {
                }
            });
        } catch (Exception e) {
    ...
    public void createCancelReceipt(int orderId, java.util.List<model.OrderItem> items, int userId) {
        getJdbi().useTransaction(handle -> {
            ...
            int receiptId = handle.createUpdate(
                    ...
                    .bind("userId", userId)
                    .bind("note", "Hoàn kho do huỷ đơn hàng #" + orderId)
                    .bind("totalAmount", totalAmount)
                    .bind("orderId", orderId)
                    .executeAndReturnGeneratedKeys()
                    .mapTo(Integer.class)
            ...
            for (model.OrderItem item : items) {
                Double price = handle
                        .createQuery(
                                "SELECT price FROM order_items WHERE order_id = :oid AND variant_id = :vid LIMIT 1")
                        .bind("oid", orderId)
                        .bind("vid", item.getVariantId())
                        .mapTo(Double.class)
                        .findOne()
