...
public class UserDao extends BaseDao {
    public User finduser(String username) {
        return getJdbi().withHandle(
                handle -> handle.createQuery("select * from users where username=:username OR email=:username")
    ...
    public User findByEmail(String email) {
        return getJdbi().withHandle(handle -> handle.createQuery("SELECT * FROM users WHERE email = :email")
                .bind("email", email)
                .mapToBean(User.class)
                .findFirst()
                .orElse(null));
    ...
    public User findByGoogleSub(String googleSub) {
        return getJdbi().withHandle(handle -> handle.createQuery("SELECT * FROM users WHERE google_sub = :googleSub")
    ...
    public boolean existsByUsername(String username) {
        return getJdbi().withHandle(h -> h.createQuery("SELECT COUNT(*) FROM users WHERE username = :u")
                .bind("u", username)
                .mapTo(int.class)
                .one()) > 0;
    }
    ...
    public void linkGoogleAccount(int userId, String googleSub, String avatarUrl, String fullName) {
        getJdbi().withHandle(h -> h.createUpdate("""
                UPDATE users
    ...
    public void insertGoogleUser(String username, String email, String passwordHash, String fullName, String googleSub,
            String avatarUrl) {
        getJdbi().withHandle(h -> h
                .createUpdate(
                        """
                                INSERT INTO users
    ...
    public void insertPendingUser(String username, String email, String password, String fullName, String birthdayStr,
            String gender,
    ...
    public void updatePendingUser(String email, String username, String password, String fullName, String birthdayStr,
            String gender,
            String otp, LocalDateTime expiredAt) {
        getJdbi().withHandle(h -> h.createUpdate(
    ...
    public void deletePendingUserByEmail(String email) {
        getJdbi().useHandle(h -> h.createUpdate("DELETE FROM users WHERE email = :email AND is_active = 0")
                .bind("email", email)
                .execute());
    }
    ...
    public void updateOtpForReset(String email, String otp, LocalDateTime expiredAt) {
        getJdbi().withHandle(h -> h.createUpdate("UPDATE users SET otp_code=:otp, otp_expired_at=:exp WHERE email=:e")
                .bind("otp", otp)
                .bind("exp", expiredAt)
                .bind("e", email)
                .execute());
    ...
    public boolean verifyOtpForReset(String email, String otp) {
        int count = getJdbi().withHandle(h -> h.createQuery(
    ...
    public void updatePassword(String email, String password) {
        getJdbi().withHandle(h -> h.createUpdate("UPDATE users SET password=:p, otp_code=NULL, otp_expired_at=NULL " +
                "WHERE email=:e")
                .bind("p", password)
                .bind("e", email)
                .execute());
    ...
    public int incrementFailedAttempts(int userId) {
        getJdbi().useHandle(
                h -> h.createUpdate("UPDATE users SET failed_login_attempts = failed_login_attempts + 1 WHERE id = :id")
                        .bind("id", userId)
                        .execute());
        return getJdbi().withHandle(h -> h.createQuery("SELECT failed_login_attempts FROM users WHERE id = :id")
                .bind("id", userId)
                .mapTo(Integer.class)
                .one());
    }
    ...
    public void resetFailedAttempts(int userId) {
        getJdbi().useHandle(h -> h.createUpdate("UPDATE users SET failed_login_attempts = 0 WHERE id = :id")
                .bind("id", userId)
                .execute());
    }
